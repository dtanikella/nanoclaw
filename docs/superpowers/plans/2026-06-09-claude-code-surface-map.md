# Claude-Code Integration Surface Map — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Produce a spike document cataloging every claude-code/claude-agent-sdk touchpoint in NanoClaw, classified as essential or incidental.

**Architecture:** Systematic investigation of three layers (build, spawn, runtime), writing each section of the spike document incrementally. Each task produces one complete section committed to git.

**Tech Stack:** Markdown document, git

---

## File Structure

- **Create:** `docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md` — the spike deliverable

No code changes. This is a research/documentation spike.

---

### Task 1: Executive Summary + Quick-Reference Table

**Files:**
- Create: `docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md`

- [ ] **Step 1: Create the spike document with the executive summary**

Write the document header and executive summary paragraph explaining what claude-code does in NanoClaw:
...
- [ ] **Step 2: Write the quick-reference table**

Append the quick-reference table after the executive summary. This table summarizes every touchpoint found across all files. Build it from the full investigation — each row has: touchpoint name, layer, file, essential/incidental.
...
```bash
git add docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md
git commit -m "docs(spike): add executive summary and quick-reference table
...
### Task 2: Flow Walkthrough — Image Build

**Files:**
- Modify: `docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md`

- [ ] **Step 1: Write the Image Build section**

Append the following section to the spike document:
...
```bash
git add docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md
git commit -m "docs(spike): add image build layer walkthrough
...
### Task 3: Flow Walkthrough — Host-Side Group Init & Spawn

**Files:**
- Modify: `docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md`

- [ ] **Step 1: Write the Host-Side Group Init section**
...
```bash
git add docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md
git commit -m "docs(spike): add host-side group init and spawn walkthrough
...
### Task 4: Flow Walkthrough — Container Boot & Provider Query

**Files:**
- Modify: `docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md`

- [ ] **Step 1: Write the container boot and provider query sections**
...
```bash
git add docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md
git commit -m "docs(spike): add container boot and poll loop walkthrough
...
### Task 5: Flow Walkthrough — SDK Query Options, Hooks, Events

**Files:**
- Modify: `docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md`

- [ ] **Step 1: Write the SDK query options section**
...
Whitelist of tools the agent can use. Includes 22 built-in tools (Bash, Read, Write, Edit, Glob, Grep, WebSearch, WebFetch, Task, TaskOutput, TaskStop, TeamCreate, TeamDelete, SendMessage, TodoWrite, ToolSearch, Skill, NotebookEdit) plus `mcp__<server>__*` patterns for all registered MCP servers.
...
```bash
git add docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md
git commit -m "docs(spike): add SDK options, hooks, and events walkthrough
...
### Task 6: Essential vs Incidental Classification Summary + Setup Touchpoints

**Files:**
- Modify: `docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md`

- [ ] **Step 1: Write the classification summary and setup touchpoints**
...
```bash
git add docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md
git commit -m "docs(spike): add classification summary and setup touchpoints
...
### Task 7: Detailed Appendix

**Files:**
- Modify: `docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md`

- [ ] **Step 1: Write the detailed appendix**
...
```bash
git add docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md
git commit -m "docs(spike): add detailed appendix
...
- [ ] **Step 1: Review document against acceptance criteria**

Check each criterion from issue #4:
1. ✅ Covers all three layers: build (Dockerfile), spawn (container-runner, session-manager, group-init), runtime (provider, poll-loop, hooks, DB tracking)
2. ✅ Each touchpoint includes file + line reference and description
3. ✅ Each touchpoint placed in context of overall message flow (flow walkthrough)
4. ✅ Each touchpoint classified as essential/incidental with rationale
5. ✅ Appendix catalogs: SDK query options, ProviderEvent types, hooks, tools, .claude/ paths, CLAUDE_CODE_* env vars

- [ ] **Step 2: Push branch**

```bash
git push -u origin no_claude
```

- [ ] **Step 3: Create PR**

```bash
gh pr create --title "Spike: Map claude-code integration surface" \
  --body "Closes #4.

Comprehensive surface map of all claude-code and claude-agent-sdk touchpoints across build, spawn, and runtime layers. Each touchpoint is documented with file:line references, placed in the message lifecycle context, and classified as essential or incidental.

## Document structure
- Executive summary + quick-reference table (40 touchpoints)
- 8-stage end-to-end flow walkthrough
- Essential vs incidental classification summary
- Detailed appendix (SDK options, events, hooks, tools, filesystem, env vars, packages)" \
  --base main
```

<function_calls>
<invoke name="create">
<parameter name="file_text"># Claude-Code Integration Surface Map — Implementation Plan
...
- [ ] **Step 1: Write the Host-Side Group Init and Spawn sections**

Append the Stage 2 and Stage 3 sections covering group-init.ts (settings.json, CLAUDE_CODE_* env vars, PreCompact shell hook, .claude-shared directory) and container-runner.ts (.claude-shared mount, CLAUDE.md composition, skill symlinks, shared CLAUDE.md mount). Include all touchpoints 2.1-2.3 and 3.1-3.4 with file:line references, code snippets, and classifications.

Full content as specified in the plan above (Task 3 from the detailed plan content).
...
### Task 4: Flow Walkthrough — Container Boot & Poll Loop

**Files:**
- Modify: `docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md`

- [ ] **Step 1: Write Stages 4 and 5**

Append Stage 4 (Container Boot — provider barrel, factory, registration) and Stage 5 (Poll Loop — continuation management, supportsNativeSlashCommands, query call, follow-up push, stale session recovery, event consumption). Include touchpoints 4.1-4.2 and 5.1-5.6 with file:line references, code snippets, and classifications.

Full content as specified in Task 4 from the detailed plan content.
...
### Task 5: Flow Walkthrough — SDK Options, Hooks, Events

**Files:**
- Modify: `docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md`

- [ ] **Step 1: Write Stages 6, 7, and 8**

Append Stage 6 (SDK Query Options — all 14 options with code snippets), Stage 7 (Hooks — PreToolUse, PostToolUse, PreCompact SDK hooks + shell hooks), and Stage 8 (Event Translation + Session Management — event mapping, transcript rotation, stale detection, auto-compact window). Include touchpoints 6.1-6.14, 7.1-7.4, and 8.1-8.4.

Full content as specified in Task 5 from the detailed plan content.
...
### Task 6: Classification Summary + Setup Touchpoints

**Files:**
- Modify: `docs/superpowers/spikes/2026-06-09-claude-code-surface-map.md`

- [ ] **Step 1: Write classification summary and setup touchpoints**

Append the Essential vs Incidental Classification section (essential capabilities table, incidental mechanisms table) and setup-layer touchpoints (verify.ts, claude-assist.ts).

Full content as specified in Task 6 from the detailed plan content.
...
Append sections A through G:
- A: All `sdkQuery()` options (16 rows)
- B: All `ProviderEvent` types (5 types)
- C: All hooks (4 SDK + 1 shell)
- D: All tools in allow/disallow lists (22 allowed + 9 disallowed)
- E: All `.claude/` filesystem conventions (6 paths)
- F: All `CLAUDE_CODE_*` env vars (9 vars)
- G: NPM packages (3 packages)

Full content as specified in Task 7 from the detailed plan content.
